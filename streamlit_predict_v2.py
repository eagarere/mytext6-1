...
with col_form:
    ...
    with st.form('user_inputs'):
        ...
        bill_length = st.number_input('喙的长度（毫米）', min_value=20.0, max_value=70.0)
        bill_depth = st.number_input('喙的深度（毫米）', min_value=10.0,max_value=25.0)
        flipper_length = st.number_input('翅膀的长度（毫米）', min_value=170.0,max_value=230.0)
        body_mass = st.number_input('身体质量（克）', min_value=2000.0,max_value=6000.0)
...
with col_logo:
    if not submitted:
        st.image('Chapter8_resources/images/rigth_logo.png', width=300)
    else:
